...
USE `brainshare_dev`
;
DROP FUNCTION IF EXISTS `brainshare_dev`.`user_status_id_for`
;
...
USE `brainshare_dev`;
DROP FUNCTION IF EXISTS `brainshare_dev`.`material_state_id_for`;
...
USE `brainshare_dev`;
DROP PROCEDURE IF EXISTS `brainshare_dev`.`recount_approval_value`;
...
USE `brainshare_dev`
;
DROP PROCEDURE IF EXISTS `brainshare_dev`.`add_commentary_mark`
;
...
USE `brainshare_dev`
;
DROP PROCEDURE IF EXISTS `brainshare_dev`.`add_material_mark`
;
...
USE `brainshare_dev`;
DROP TRIGGER IF EXISTS `brainshare_dev`.`user_update_referred_tables_user_id_to_zero`;
USE `brainshare_dev`;
...
USE `brainshare_dev`;
DROP TRIGGER IF EXISTS `brainshare_dev`.`user_cleanup_avatar_object_storage_file`;
USE `brainshare_dev`;
...
USE `brainshare_dev`;
DROP TRIGGER IF EXISTS `brainshare_dev`.`material_cleanup_content_data`;
USE `brainshare_dev`;
...
USE `brainshare_dev`;
DROP TRIGGER IF EXISTS `brainshare_dev`.`commentary_cleanup_content_data`;
USE `brainshare_dev`;
...
USE `brainshare_dev`;
DROP TRIGGER IF EXISTS `brainshare_dev`.`material_attachment_cleanup_object_storage_file`;
USE `brainshare_dev`;
...
START TRANSACTION;
USE `brainshare_dev`;
INSERT IGNORE INTO `brainshare_dev`.`user_status` (`id`, `status_name`, `description`, `low_border`) VALUES
  (0, 'WAS_REMOVED',
   'Some user was removed, but some of his materials, or comments, or other data need to be still existing', 0);
...
START TRANSACTION;
USE `brainshare_dev`;
INSERT IGNORE INTO `brainshare_dev`.`user` (`id`, `first_name`, `last_name`, `avatar`, `email`, `login`, `password`, `registration_date`, `social_links_json`, `ratio`, `penaly_count`, `status_id`)
...
START TRANSACTION;
USE `brainshare_dev`;
INSERT IGNORE INTO `brainshare_dev`.`material_state` (`id`, `state_name`, `description`, `low_border`)
...
START TRANSACTION;
USE `brainshare_dev`;
INSERT IGNORE INTO `brainshare_dev`.`material_format` (`id`, `format_name`, `description`)
...
START TRANSACTION;
USE `brainshare_dev`;
INSERT IGNORE INTO `brainshare_dev`.`section` (`id`, `super_section_id`, `section_name`, `description`)
...
START TRANSACTION;
USE `brainshare_dev`;
INSERT IGNORE INTO `brainshare_dev`.`notification_type` (`id`, `type_name`, `description`)
